...
C++ 版支持独立封面与多图循环播放，可批量拖入或选择播放图片，并移除选中项；保留路径规范化和防覆盖。图片按列表顺序播放，每张 100 ms，画布采用第一张播放图的尺寸，其他图片等比例适配并居中留白。单张播放图仍使用双帧模式。支持系统 WIC 可解码的 PNG、JPEG、BMP、GIF、TIFF；WebP 取决于系统是否安装对应编解码器。封面使用 WIC 等比例缩放；隐藏图保留解码后的分辨率和像素。两种版本的缩放算法和压缩结果不保证字节一致。
...
```powershell
./dist/QQ-APNG-Disguise-CPP.exe "封面.png" "播放图1.png" "播放图2.png" "播放图3.png" "结果.png"
```
...
输出是一个带独立静态封面的 APNG 文件。C++ 版有多张播放图时，每张对应一个完整动画帧，持续 100 ms，以替换方式显示，循环播放；封面不参与播放。只有一张播放图时，两种版本均使用以下结构：
